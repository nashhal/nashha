/* news-loader.js
   يقرأ data/news.json دون تغيير بنيته، ويعرض الأخبار بصياغة نشهل المستقلة.
   أسماء المصادر محفوظة للتحقق الداخلي ولا تظهر في واجهة الزائر.
*/
(function () {
  'use strict';
  ...
  function normalize(item = {}) {
    ...
    return {
      title, summary, sourceName, sourceUrl, published, category,
      status: clean(item.status || 'published'),
      confidence: clean(item.confidence || ''),
      platform: clean(item.platform || 'official'),
      city,
  ...
  function metaLine(item) {
    return `<div class="meta-line"><span>${escapeHtml(item.category)}</span><span class="dot"></span><span>${escapeHtml(timeAgo(item.published))}</span></div>`;
  }

  function placeholder(label = 'نشهل') {
  ...
  function setupSearch() {
    ...
    form.addEventListener('submit', event => {
      ...
      results.innerHTML = q && matches.length
        ? matches.map(item => `<a href="${escapeHtml(item.sourceUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(item.title)}</a>`).join('')
        : (q ? '<span>لا توجد نتائج مطابقة</span>' : '');
    });
  }
